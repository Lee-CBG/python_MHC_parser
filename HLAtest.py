...
This uses orignal files from https://github.com/ANHIG/IMGTHLA
...
def main():
    ...
    '''
    # version with unknown * values as part of the final count
    ...
    for i, element in enumerate(dfs):
        ...
        count = (element.nunique(dropna=True)).sum() 
        element = element.fillna('*')  # fill every element with unkown value wiht NaN for ease of use 
       # col_count_df = pd.DataFrame(df_el.nunique() - 1).unstack().sum(1) # this is if we don't want to count the first row, but we do so comment out 
        total+=count # sum total count for this file 
